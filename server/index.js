...
app.get('/api/locations', async (req, res) => {
    try {
        const kpiDataCollection = mongoose.connection.collection('kpiData2');
        const locations = await kpiDataCollection.find({}, { projection: { _id: 0, lat: 1, lng: 1, city: 1, KPI: 1 } }).toArray();
        res.json(locations);
    } catch (error) {
...
// Endpoint to get data for a specific city from kpiData collection
app.get('/api/data/:city', async (req, res) => {
    const city = req.params.city;
    try {
        const kpiDataCollection = mongoose.connection.collection('kpiData2');
        const data = await kpiDataCollection.find({ city }).toArray();
        console.log('Data fetched for city:', city, data);
        res.json(data);
    } catch (error) {
